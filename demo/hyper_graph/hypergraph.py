...
if __name__ == '__main__':
    ...
    size_embeddings = 512
    hgnn_hidden = 512
    embeddings = ''  # model output 是resnet最后一层经过线性层得输出,为(bs,512),一般还有self.lnorm = nn.LayerNorm(embedding_size, elementwise_affine=False).cuda()
    target = ''
    # Hypergraph construction & distribution loss
    dist_loss, H = d2hg(embeddings, target)

    hnmp = HGNN(nb_classes=nb_classes,
                sz_embed=size_embeddings,
                hidden=hgnn_hidden)
    out = hnmp(embeddings, H)