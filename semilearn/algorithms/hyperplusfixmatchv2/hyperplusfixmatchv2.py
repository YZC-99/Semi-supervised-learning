    ...
    def train_step(self, x_lb, y_lb, in_clinical, x_ulb_w, x_ulb_s_0, x_ulb_s_1, ex_clinical):
        ...
        with self.amp_cm():
            ...
            if self.args.loss == 'bce':
                ...
                H1, node_embeddings1, indices_map = construct_hypergraph_single_label_whole(clinical[:num_lb + num_ulb],
                                                                                            label_classes,
                                                                                            feats[:num_lb + num_ulb])
                # 构建超图 H2（针对 x_ulb_s_0）
                pred_classes_ulb_s0 = logits_x_ulb_s_0.argmax(dim=1)
                H2, node_embeddings2, _ = construct_hypergraph_single_label_whole(ex_clinical_ulb_s0,
                                                                                  pred_classes_ulb_s0,
                                                                                  feats_x_ulb_s_0)

            hyper_out = self.model.stage2_forward(node_embeddings1, H1, node_embeddings2, H2)
            hyper_whole_out, hyper_strong_out = hyper_out['hyper_logits'], hyper_out['hyper_logits2']
            ...
            # 计算DDL(distribution difference for labeled data)
            DL1, DL2 = F.softmax(logits_x_lb,dim=1), F.softmax(hyper_whole_out[:num_lb],dim=1)
            DDL = F.kl_div(DL1.log(), DL2, reduction='batchmean')

            # 计算DDU1_w(distribution difference for unlabeled data)
            DU1_w, DU2_w = F.softmax(logits_x_ulb_w,dim=1), F.softmax(hyper_whole_out[num_lb:],dim=1)
            # 计算每个未标记样本的 DDU_w
            DDU_w = F.kl_div(DU1_w.log(), DU2_w, reduction='none').sum(dim=1) + \
                    F.kl_div(DU2_w.log(), DU1_w, reduction='none').sum(dim=1)

            # 计算DDU1_s(distribution difference for unlabeled data)
            DU1_s, DU2_s = F.softmax(logits_x_ulb_s_0,dim=1), F.softmax(hyper_strong_out,dim=1)
            # 计算每个未标记样本的 DDU_s
            DDU_s = F.kl_div(DU1_s.log(), DU2_s, reduction='none').sum(dim=1) + \
                    F.kl_div(DU2_s.log(), DU1_s, reduction='none').sum(dim=1)

            # 标准化 DDU_w 和 DDU_s
            DDU_w_mean = DDU_w.mean()
            DDU_w_std = DDU_w.std()
            DDU_s_mean = DDU_s.mean()
            DDU_s_std = DDU_s.std()

            DDU_w_normalized = (DDU_w - DDU_w_mean) / DDU_w_std
            DDU_s_normalized = (DDU_s - DDU_s_mean) / DDU_s_std

            # DDL 是标量，可以将其标准化为与 DDU_w 和 DDU_s 相同的尺度
            DDL_normalized = (DDL - DDU_w_mean) / DDU_w_std  # 或根据需要选择其他参考统计量

            # 计算每个样本与 DDL 和 DDU_s 的差异
            diff_w = torch.abs(DDU_w_normalized - DDL_normalized)
            diff_s = torch.abs(DDU_s_normalized - DDL_normalized)
            ...
            # 综合差异
            combined_diff = weight_w * diff_w + weight_s * diff_s

            # 定义置信度评分，差异越小，置信度越高
            confidence_scores = torch.exp(-combined_diff)
            # 定义一个阈值，例如 0.9，可以根据需要动态调整
            threshold = 0.9
            ...
            # 由于全图的构建已经使用了未标记的伪标签，因此这里将使用全图的伪标签去监督小的超图。
            # 但是如果直接使用全图的伪标签
            pseudo_labels = torch.argmax(part_hyper_whole_out, dim=1)
            pseudo_labels = F.one_hot(pseudo_labels, num_classes=self.args.num_classes)
            pseudo_labels = pseudo_labels.to(part_hyper_whole_out.device)
            hyper_class_loss = self.consistency_loss(logits_x_ulb_s_0,
                                               pseudo_labels,
                                               self.args.loss,
                                               mask=high_confidence_mask)
